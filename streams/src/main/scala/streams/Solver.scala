...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial == Stream.Empty) return Stream.Empty
    ...
    val updatedExplored = explored ++ newNeighbors.map(_._1).toSet 
    newNeighbors #::: from(initial.tail #::: newNeighbors, updatedExplored)
  }
  ...
  lazy val solution: List[Move] =  pathsToGoal match {
    ...
    case (block, history) #:: other => history.reverse
  }
}
